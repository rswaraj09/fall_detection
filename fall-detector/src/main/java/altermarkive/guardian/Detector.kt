...
class Detector private constructor(private val initialContext: Context?, private val number: String = "") : SensorEventListener {
    var context: Guardian? = null

    companion object {
        // Initialize with null context and empty string, will be properly set in instance()
        @Volatile
        internal var singleton: Detector = Detector(null)
        ...
        fun log(level: Int, entry: String) {
            // Replace BuildConfig.DEBUG with true for development
        ...
        /**
         * Get the singleton detector's buffers
         */
        fun getBuffers(): Buffers {
            return singleton.buffers
        }
    }
    ...
    private fun process() {
        ...
        if (0 == timeoutImpact) {
            ...
            if (LYING_AVERAGE_Z_LPF < (sum / count)) {
                lying[at] = 1.0
                // Try to get a valid context either from the class or the initialContext
                val validContext = context ?: initialContext
                if (validContext != null) {
                    Guardian.say(validContext, android.util.Log.WARN, TAG, "Detected a fall")
                    alert(validContext)
                } else {
                    Log.e(TAG, "No valid context available to handle fall detection")
                }
            }
        }
    }
    ...
    private fun initiateSensor(context: Guardian) {
        ...
        if (sensor != null) {
            ...
            log(android.util.Log.ERROR, "No accelerometer sensor found!")
        }
    }

    private fun alert(context: Context) {
        Log.i(TAG, "Fall detected - starting voice confirmation")
        
        // Get a valid context - either the one passed in or the initialContext if available
        val validContext = context ?: initialContext
        
        if (validContext == null) {
            Log.e(TAG, "No valid context available, cannot start voice confirmation")
            return
        }
        
        // Get the emergency number from shared preferences
        val emergencyNumber = "9022442848"
        
        // Save the emergency number to Contact preferences to ensure it's accessible 
        try {
            Contact[validContext] = emergencyNumber
            Log.i(TAG, "Emergency number ($emergencyNumber) saved to preferences")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to save emergency number: ${e.message}")
        }
        
        // Use the new VoiceAssistant to handle the fall confirmation flow
        VoiceAssistant.getInstance(validContext).startFallConfirmation { isUserOkay ->
            Log.i(TAG, "Voice confirmation result: user is ${if (isUserOkay) "OKAY" else "NOT OKAY"}")
            
            if (!isUserOkay) {
                // Only trigger the emergency alert if the user didn't confirm they're okay
                Log.i(TAG, "User did NOT confirm they're okay - triggering emergency alert")
                // Log the emergency number being used
                Log.i(TAG, "Using emergency contact number: $emergencyNumber")
                
                // Check if the emergency number is properly set
                val storedContact = Contact[validContext]
                Log.i(TAG, "Stored emergency contact in preferences: $storedContact")
                
                // First try with direct number
                Alarm.alert(validContext, emergencyNumber)
                
                // Also try the standard alert method as a backup
                if (storedContact.isNullOrEmpty()) {
                    Log.i(TAG, "No contact found in preferences, using direct alert with emergency number")
                    Alarm.alert(validContext, emergencyNumber)
                } else {
                    Log.i(TAG, "Using alert with stored contact from preferences")
                    Alarm.alert(validContext)
                }
            } else {
                Log.i(TAG, "User confirmed they're okay - no emergency alert needed")
                // Optionally play a confirmation sound or show a message to confirm no alert was sent
            }
        }
    }
}